...
async function main() {
    ...
    if (process.env.NODE_ENV === 'production' || process.env.PASSENGER_APP_ENV) {
        logger_1.log.info('boot.hosting.detected', { env: process.env.NODE_ENV, passenger: process.env.PASSENGER_APP_ENV });
    ...
    try {
        ...
        if (e.message && e.message.includes('listen() was called more than once')) {
            logger_1.log.info('boot.listen.hosting_managed', { note: 'Hosting environment managing port binding' });
...
main().then(app => {
    if (app) {
        logger_1.log.info('boot.complete', { message: 'Server initialization complete' });
