...
exports.readWindowStats = readWindowStats;
...
function readWindowStats() {
    const now = new Date();
    const baseDir = path_1.default.join(config_js_1.CONFIG.LOG_DIR, 'json');
    const days = [];
    for (let i = config_js_1.CONFIG.STATS_WINDOW_DAYS - 1; i >= 0; i--) {
    ...
    const sessionsFrustrated = [];
    const mask = (anon) => {
        if (!anon || anon.length < 6)
            return 'Student #—';
        return `Student #${anon.slice(-6)}`;
    };
    for (let di = 0; di < days.length; di++) {
        const day = days[di];
        const file = path_1.default.join(baseDir, `${day}.jsonl`);
        ...
        for (const e of arr) {
            ...
            if (evt === 'java.run.completed') {
                ...
                if (!dailyLearningOutcomes[keyDay])
                    dailyLearningOutcomes[keyDay] = {};
                ...
                if (sess.exit === 0)
                    sessionsSuccess.push(sess);
                if (sess.exit === 130)
                    sessionsFrustrated.push(sess);
            }
    ...
    sessionsSuccess.sort((a, b) => {
        ...
        if (bi !== ai)
            return bi - ai; // interactive first
    ...
    sessionsFrustrated.sort((a, b) => {
        ...
        const bi = b.interactive ? 1 : 0;
        if (ai !== bi)
            return ai - bi; // non-interactive first
        return a.durationMs - b.durationMs; // shortest durations first
    });
    return {
        from: days[0],
        to: days[days.length - 1],
        windowDays: config_js_1.CONFIG.STATS_WINDOW_DAYS,
        total,
