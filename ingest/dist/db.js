...
const _initCounters = (async () => { try {
    await ensureCounterTables();
}
catch { /* ignore until pool ready */ } })();
export async function dbUpsertInstallCounters(ev, geo) {
    if (!dbEnabled())
        return;
    if (!pool)
        pool = buildPool();
    ...
    try {
        ...
        for (const q of queries) {
            if (CONFIG.DEBUG_DB)
                log.debug('[db] upsert counter', { sql: q.text, values: q.values });
...
export async function dbReadInstallStats(windowDays) {
    if (!dbEnabled())
        return null;
    if (!pool)
        pool = buildPool();
    ...
    try {
      ...
      `, [from, to]),
        ...
        const objFrom = (rows) => Object.fromEntries(rows.map(r => [r.k, Number(r.v || 0)]));
        return {
            from: fromStr,
            to: toStr,
            windowDays,
