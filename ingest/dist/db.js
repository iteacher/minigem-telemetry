...
export async function dbReadStats(windowDays, fromS, toS) {
    ...
    try {
        ...
        if (CONFIG.DEBUG_DB)
            log.debug('[db] stats date range', { from: from.toISOString(), to: to.toISOString(), queryFrom: queryFrom.toISOString(), queryTo: queryTo.toISOString(), windowDays });
        // Totals and KPIs
        const qTotalsSql = `
      select
        count(*)::int as total,
  ...
  `;
        ...
        if (CONFIG.DEBUG_DB)
            log.debug('[db] stats.qTotals', { sql: qTotalsSql, params: [queryFrom, queryTo] });
