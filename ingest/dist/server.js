...
async function main() {
    const app = Fastify({ logger: true, bodyLimit: CONFIG.MAX_BODY, trustProxy: true });
    await initGeo();
    await dbInit();
    ...
    app.post('/t', async (req, reply) => {
        try {
            ...
            if (schema !== 'jwc.v1') {
                req.log.warn({ got: raw?.schema }, 'schema not supported');
            ...
            for (const evRaw of batchRaw) {
                ...
                if (!ev || !validateEvent(ev)) {
                    skipped++;
                    req.log.warn({ evRaw }, 'event skipped: invalid');
                    continue;
                }
                try {
                    if (dbEnabled())
                ...
                try {
                    appendLine(lineForEvent(ev, geo));
                }
                catch { }
                try {
                    appendJsonEvent(ev, geo);
                }
                catch { }
                accepted++;
            }
