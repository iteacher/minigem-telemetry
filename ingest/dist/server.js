...
async function main() {
    ...
    try {
        ...
        logger_1.log.error('boot.geo.failed', String(e));
        console.error('GEO INIT FAILED:', e);
        // Continue without geo - it's not critical
    }
    try {
        logger_1.log.info('boot.store.start');
        await (0, store_1.storeInit)();
        logger_1.log.info('boot.store.done', { file: (0, store_1.pathToStore)() });
    }
    catch (e) {
        logger_1.log.error('boot.store.failed', String(e));
    ...
    try {
        ...
        logger_1.log.error('boot.register.rateLimit.failed', String(e));
