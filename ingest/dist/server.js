...
async function main() {
    ...
    app.addHook('onRequest', async (req) => { logger_1.log.info('req', { method: req.method, url: req.url, ip: req.ip }); });
    app.addHook('onResponse', async (req, reply) => { logger_1.log.info('res', { method: req.method, url: req.url, status: reply.statusCode }); });
    ...
    try {
        logger_1.log.info('boot.listen.start', { port: config_1.CONFIG.PORT, host: '0.0.0.0' });
        await app.listen({ port: config_1.CONFIG.PORT, host: '0.0.0.0' });
        logger_1.log.info('boot.listen.ok', { url: `http://0.0.0.0:${config_1.CONFIG.PORT}` });
    }
    catch (e) {
